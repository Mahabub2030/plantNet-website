...
async function run() {
  try {
    // collction start here
    const db = client.db("plants-secssions");
    const usersColloection = db.collection("users");
    ...
    app.post("/users/:email", async (req, res) => {
      ...
      const isExist = await usersColloection.findOne(query);
      if (isExist) {
        return res.send(isExist);
      }
      const result = await usersColloection.insertOne({
        ...user,
        role: "customer",
        timestamp: Date.now(),
      });
    ...
    app.patch("/users/:email", async (req, res) => {
      ...
      if (!user || user?.status === "requested")
        return res
          .status(404)
          .send("You have allready requsete wating for admin.");
      const updateDoc = {
        $set: {
          status: "requested",
        },
      };
      const result = await usersColloection.updateOne(query, updateDoc);
      console.log(result);
      res.send(result);
    });
    // get all user data
